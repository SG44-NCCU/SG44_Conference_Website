...
function toEnglishDate(isoDate: string) {
  const fmt = new Intl.DateTimeFormat('en-US', { timeZone: TW_TZ, year: 'numeric', month: 'long', day: 'numeric' })
  return fmt.format(new Date(isoDate))
}

function toEnglishDateTime(isoDate: string) {
...
const getStyles = (lang: string) => `
  * { margin: 0; padding: 0; box-sizing: border-box; }
  body { 
    font-family: ${lang === 'zh' ? "'標楷體', 'KaiTi', 'DFKai-SB', 'Times New Roman', serif" : "'Times New Roman', serif"}; 
    font-size: 13pt; color: #1a1a1a; background: white; padding: 20mm; line-height: 1.9; 
  }
  ...
  table.info-table td:first-child { white-space: nowrap; font-weight: bold; width: ${lang === 'zh' ? '110pt' : '130pt'}; }
  ...
  table.detail-table td.label { font-weight: bold; background: #f5f5f5; width: ${lang === 'zh' ? '130pt' : '150pt'}; white-space: nowrap; }
  ...
  .digital-seal p { font-size: 11pt; line-height: 1.6; }
...
export default async function AuthorizationPrintPage({
  ...
  const { lang = 'zh' } = await searchParams
  ...
  let topicLabelZH = doc.subTopic ? SUB_TOPICS_MAP[doc.subTopic as string] : (doc.specialSession as string)
  let topicLabelEN = doc.subTopic ? SUB_TOPICS_MAP[doc.subTopic as string] : (doc.specialSession as string)
  if (splitMatch) {
    topicLabelZH = splitMatch[1]
    topicLabelEN = splitMatch[2]
  }

  topicLabelZH = topicLabelZH || '—'
  topicLabelEN = topicLabelEN || '—'

  const authDateISO = (doc.authorizationDate as string) ?? doc.createdAt
  const authDateFormattedZH = toROCDate(authDateISO)
  const authDateTimeFormattedZH = toROCDateTime(authDateISO)
  const authDateFormattedEN = toEnglishDate(authDateISO)
  const authDateTimeFormattedEN = toEnglishDateTime(authDateISO)

  const idNumber = (doc.authorizationIdNumber as string) ?? (lang === 'zh' ? '（未提供）' : '(Not Provided)')
  const address = (doc.authorizationAddress as string) ?? (lang === 'zh' ? '（未提供）' : '(Not Provided)')
  const phone = (doc.authorizationPhone as string) ?? (lang === 'zh' ? '（未提供）' : '(Not Provided)')

  return (
    <html lang={lang === 'zh' ? 'zh-TW' : 'en'}>
      <head>
        <meta charSet="utf-8" />
        <title>{lang === 'zh' ? `論文授權書 - ${doc.title}` : `Paper Authorization Agreement - ${doc.title}`}</title>
        <style dangerouslySetInnerHTML={{ __html: getStyles(lang) }} />
      </head>
      <body>
        <PrintButton />
        ...
        {lang === 'zh' ? (
          <>
            <h1 style={{ fontFamily: "'標楷體', 'KaiTi', 'DFKai-SB', 'Times New Roman', serif" }}>
              「第44屆測量及空間資訊研討會」徵稿公告
            </h1>
            <section style={{ fontFamily: "'標楷體', 'KaiTi', 'DFKai-SB', 'Times New Roman', serif" }}>
            ...
            <h2 style={{ fontFamily: "'標楷體', 'KaiTi', 'DFKai-SB', 'Times New Roman', serif" }}>
            ...
            <section style={{ fontFamily: "'標楷體', 'KaiTi', 'DFKai-SB', 'Times New Roman', serif" }}>
              ...
              <table className="detail-table">
                <tbody>
                  ...
                  <tr><td className="label">投稿子題</td><td>{topicLabelZH}</td></tr>
                </tbody>
              </table>
            </section>
            ...
            <section style={{ fontFamily: "'標楷體', 'KaiTi', 'DFKai-SB', 'Times New Roman', serif" }}>
              ...
              <div className="digital-seal">
                ...
                <p style={{ marginTop: '6pt' }}>
                  授權確認時間：<strong>{authDateTimeFormattedZH}</strong>（中華民國 {authDateFormattedZH}）
                </p>
          ...
          <>
            <h1>The 44th Surveying and Geoinformatics Conference - Call for Papers</h1>
            <section>
              <table className="info-table"><tbody>
                <tr><td>Date</td><td>August 20 (Thu) to 21 (Fri), 2026</td></tr>
                <tr><td>Venue</td><td>National Chengchi University, College of Law (No. 64, Sec. 2, Zhinan Rd., Wenshan Dist., Taipei City)</td></tr>
                <tr><td>Theme</td><td>Smart Surveying × Resilient Future</td></tr>
                <tr>
                  <td>Sub-topics</td>
                  <td>
                    <ol style={{ paddingLeft: '16pt' }}>
              ...
              <section>
                <h3>Submission Guidelines & Registration</h3>
                <ol className="no-list">
                  <li><span className="num">1.</span><span>For details, please refer to the SG44 website.</span></li>
                  <li><span className="num">2.</span><span>Upload all required forms via the web system.</span></li>
                  <li><span className="num">3.</span><span>Provide the grant number if the paper is funded.</span></li>
                  <li><span className="num">4.</span><span>The first author must be the presenter. Students presenting as the first author are eligible for the "Best Student Paper Award".</span></li>
                  <li><span className="num">5.</span><span>All attendees must register via the web system.</span></li>
                </ol>
              </section>
            </section>
            ...
            <h2>Paper Authorization Agreement (SG44)</h2>

            <section>
              <h3>Part I: Author & Paper Information</h3>
              <table className="detail-table">
                <tbody>
                  <tr><td className="label">Submitter / Presenter</td><td>{displayName}</td></tr>
                  <tr><td className="label">ID/Passport No.</td><td>{idNumber}</td></tr>
                  <tr><td className="label">Address</td><td>{address}</td></tr>
                  <tr><td className="label">Phone</td><td>{phone}</td></tr>
                  <tr><td className="label">Paper Title</td><td>{String(doc.title)}</td></tr>
                  <tr><td className="label">Co-Authors</td><td>{coAuthors}</td></tr>
                  <tr><td className="label">Sub-topic</td><td>{topicLabelEN}</td></tr>
                </tbody>
              </table>
            </section>

            <section>
              <h3>Part II: Authorization Statement</h3>
              <div className="statement">
              ...
              <div className="digital-seal">
                <p className="seal-title">[ Digital Authorization Record ]</p>
                <p>This authorization agreement has been digitally confirmed by the submitter via the SG44 submission system, which holds the same legal validity as a handwritten signature.</p>
                <p style={{ marginTop: '6pt' }}>
                  Confirmation Time: <strong>{authDateTimeFormattedEN}</strong> (Taiwan Local Time)
                </p>
                <p>Confirmed by: <strong>{displayName}</strong></p>
              </div>
            </section>
          </>
        )}
